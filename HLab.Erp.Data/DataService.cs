...
public class DataService : IDataService, IService
{
    ...
    public IEnumerable<string> Connections => _connectionStrings ??= _options.GetSubList("", "Connections", null, "registry");

    string _connectionString;

    public void SetConnectionStringInteractive(string message)
    {
        _connectionString = _getConnectionString(message,_connectionString);
    }
    ...
    public string ConnectionString
    {
        get
        {
            ...
            {
                // Retrieve connection string from registry
                ...
                if (string.IsNullOrWhiteSpace(_connectionString))
                {
                    SetConnectionStringInteractive("{Please supply user database credentials}");
                }

                if (!string.IsNullOrEmpty(_connectionString))
                {
                    const string key = "DummyPasswordToStoreDatabaseCredentials";
                    var saved = false;
                    while (!_connectionString.Contains("Password"))
                    {
                        saved = true;
                        _connectionString = AesOperation.Decrypt(key, _connectionString);
                    }

                    if (!saved)
                    {
                        var save = AesOperation.Encrypt(key, _connectionString);
                        _options.SetValue(path, "connection", save);
                    }
                }
            }
...
//        public DbTransaction BeginTransaction() => Get().Transaction;
    ...
    Func<string,string,string> _getConnectionString = null;

    public void SetConfigureAction(Func<string,string,string> action)
    {            
